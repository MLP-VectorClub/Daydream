...
export const SpriteGeneratorCustomizer: VFC<PropTypes> = ({ options, setOptions, colorMap, setColorMap }) => {
  const setSpecies = useCallback((wing: boolean, horn: boolean) => setOptions({ ...options, wing, horn }), [options, setOptions]);
  const setBodyType = useCallback((body: SpriteGeneratorBodyOptions) => {
    const eye = body === SpriteGeneratorBodyOptions.MALE ? SpriteGeneratorEyeOptions.EYES_MALE_12 : SpriteGeneratorEyeOptions.EYES_FEMALE_1;
    setOptions({ ...options, body, eye });
  }, [options, setOptions]);
  const setEye = useCallback((eyeNumber: 1 | 2 | 3) => {
  ...
  const setGradientStops = useCallback((gradientStops: SpriteGeneratorOptions['gradientStops']) => setOptions({
    ...options,
    gradientStops,
  }), [options, setOptions]);
  return (
